...
func TestWithChartDepsTest(t *testing.T) {
	...
	var dep1ChartFound, dep2ChartFound, grafanaChartFound, redisChartFound, postgresChartFound bool
	var chartContent, dep1ChartContent, dep2ChartContent, grafanaChartContent, redisChartContent, postgresChartContent string
	...
	for {
		...
		if header.Name == "with_chart_deps/charts/grafana/Chart.yaml" {
		...
		if header.Name == "with_chart_deps/charts/redis/Chart.yaml" {
		...
		if header.Name == "with_chart_deps/charts/postgresql/Chart.yaml" {
	...
	if len(chart.Dependencies) != 5 {
		t.Fatalf("Expected 5 dependencies in main Chart.yaml, but found %d", len(chart.Dependencies))
	}
